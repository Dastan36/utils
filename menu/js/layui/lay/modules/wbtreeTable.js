...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    TreeTable.prototype.bindEvents = function () {
        ...
        $allBody.off('click.fold').on('click.fold', '.ew-tree-pack', function (e) {
            ...
            layui.event.call(this, MOD_NAME, 'toggleRow(' + components.filter + ')', {type: open ? 'close' : 'open', tr: $tr, data: data});
        });
    ...
    /** 重新设置高度 */
    TreeTable.prototype.resetHeight = function (h) {
        var options = this.options;
        var components = this.getComponents();
        var tbFilter = options.elem.substring(1);  // 表格的filter
        
        this.options.height = h || this.options.height;
        
        if (options.height.indexOf('full') === 0) {  // 差值高度
            var h = components.$toolbar.outerHeight() + components.$tHeadGroup.outerHeight() + 1;
            if (options.height.indexOf('full-') === 0) {
                h += parseFloat(options.height.substring(5));
            } else {
                h -= parseFloat(options.height.substring(5));
            }
            components.$tBodyGroup.append([
    ...
    var tt = {
        ...
        resetHeight: function(id, h) {
            _instances[id].resetHeight(h);
        },
        getDataByTr: function(id, tr) {
            return _instances[id].getDataByTr(tr);
        }
    };
