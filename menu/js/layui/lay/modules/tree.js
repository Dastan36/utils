 ...
 layui.define("form", function(e) {
 		...
 		l = function() {
 			...
 			return l.that[a] = e, l.config[a] = i, {
 				...
 				getNodeById: function(){
 					return e.getNodeById.call(e, i)
 				},
 				getTreeData: function(){
 					return e.getTreeData.call(e)
 				}
 			}
 		},
 		t = "layui-hide",
 	...
 	}, g.prototype.children = function(n1,n2) {
 		var e = this;
 		e.setChildrenData(e.config.data,n1,n2);
 	}, g.prototype.setChildrenData = function(n0,n1,n2) {
 		var e = this;
 		var c = i('#'+e.config.selectId);
 		layui.each(n0,function(a,r){
 			var b = c.find('div[data-id=' + r.id + ']').hasClass(v);
 			r.spread = b;
 			if (e.config.accordion === !0) {
 				r.spread = !1;
 		...
 		layui.each(l, function(a, l) {
 			var c = l.children,//&& l.children.length > 0,
 				...
 				h = i(['<div data-id="' + l.id + '" class="layui-tree-set' + (l.spread ? " layui-tree-spread" : "") + (l.checked ? " layui-tree-checkedFirst" : "") + '">', "<div " + (r.drag && !l.fixed ? r.leafDrag && c ? '' : 'draggable="true"' : "") + ' class="layui-tree-entry">', '<div class="layui-tree-main">', function() {
 					return r.showLine ? c ? '<span class="layui-tree-iconClick layui-tree-icon"><i class="layui-icon ' + (l.spread ? "layui-icon-subtraction" : "layui-icon-addition") + '"></i></span>' : '<span class="layui-tree-iconClick"><i class="layui-icon '+(l.icon&&l.icon!=""?l.icon:"layui-icon-file")+'"></i></span>' : '<span class="layui-tree-iconClick"><i class="layui-tree-iconArrow ' + (c ? "" : t) + '"></i></span>'
 				}(), function() {
...
// 			var a = e.children("." + y),
 			...
 			if (aa[0]) {
 				...
 				if (e.hasClass(v)) e.removeClass(v), aa.slideUp(200), n.removeClass(h).addClass(o);
 				else if (e.addClass(v), aa.slideDown(200), n.addClass(h).removeClass(o), r.accordion) {
 					var l = e.siblings("." + s);
// 					l.removeClass(v), l.children("." + y).slideUp(200), l.find(".layui-tree-icon").children(".layui-icon").removeClass(h).addClass(o)
 					l.removeClass(C), l.find('.'+y).slideUp(200).find("." + s).removeClass(C), l.find(".layui-tree-icon").children(".layui-icon").removeClass(h).addClass(o)
 				}
 			} else g = "normal"
 		}), m.on("click", function() {
 			var n = i(this);
 			...
 			if ("add" == f) {
				if (n.config.editDefaultHandle === false) { // 不执行默认操作
 				...
 				var b = r.operate && r.operate(x),
 					w = {};
 				if(b=='myTreeFunFL'){
 					myTreeFunFL('add',x.data);
 					return;
 				}
 				if(b=='myTreeFunML'){
					myTreeFunML('add',x.data);
					return;
				}
				...
				if (n.config.editDefaultHandle === false) { // 不执行默认操作
 				...
 				var bfun = r.operate && r.operate(x);
				if(bfun=='myTreeFunFL'){
					myTreeFunFL('update',x.data);
					return;
				}
				if(bfun=='myTreeFunML'){
					myTreeFunML('update',x.data);
					return;
				}
				...
				if (n.config.editDefaultHandle === false) { // 不执行默认操作
 				...
 				var bfun = r.operate && r.operate(x);
				if(bfun=='myTreeFunFL'){
					myTreeFunFL('remove',x.data);
					return;
				}
				if(bfun=='myTreeFunML'){
					myTreeFunML('remove',x.data);
					return;
				}
 	...
 	},g.prototype.getNodeById = function(id) {
 		var e = this,
			a = e.config;
 		return getLayTreeNodeById(e.config.data,id);
 	...
 	},r.getNodeById = function(e,i) {
 		var a = l.that[e];
 		return a.getNodeById(i)
 	},r.getTreeData = function(e) {
 		var a = l.that[e];
 		return a.getTreeData()
 	},r.render = function(e) {
 		var i = new g(e);
 		return l.call(i)
 	}, e(n, r)
 });
 function getLayTreeNodeById(data,id){
	 var res;
	 for(var i =0;i<data.length;i++){
		 ...
		 if(data[i].children && data[i].children.length>0){
			 var cdres = getLayTreeNodeById(data[i].children,id);
			 if (cdres){
				 res =cdres;break
			 }
		 }
