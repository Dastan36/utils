 ...
 */
layui.config({
	base: 'includesUnPack/layui/lay/modules/' // {/}的意思即代表采用自有路径，即不跟随 base 路径
}).define(['jquery','layer','form', 'wbUtil'], function(exports) {
    var $ = layui.$,
        layer = layui.layer,
        form = layui.form,
        wbUtil = layui.wbUtil;
    ...
    var LI_NAV_CHILD = "dtree-nav-ul-sid", LI_NAV_ITEM = "dtree-nav-item",
        ...
        LI_DIV_TEXT_CLASS = "t-click", UL_ROOT="dtree", LI_DIV_ORDER_CLASS="dtree-order",
    ...
    var AjaxHelper = {
        request : function(config) {
            var data = config.data ? $.extend({}, config.data, {basicData: ''}) : {};
...
//			level: "level",			//层级
    ...
    DTree.prototype.init = function(){
        ...
        if(_this.data) {
            ...
            var index = _this.load ? wbUtil.showLoading(_this.obj) : "";
    ...
    // 加载子节点
    DTree.prototype.getChild = function($div, data, cb) {
    ...
    DTree.prototype.getDom = function(treeId, parentId, title, fmtTitle, last, ficonClass, iconClass, checkArr, level, spread, disabled, hide) {
        ...
        return {
            ...
            order: function(flag) { // 排序图标
        		return [
        			'<span class="' + LI_DIV_ORDER_CLASS + '">',
            			'<i class="wb wb-top" title="置顶" type="top"></i>',
            			'<i class="wb wb-bottom" title="置底" type="bottom"></i>',
            			'<i class="fa fa-long-arrow-up" title="上移" type="up"></i>',
            			'<i class="fa fa-long-arrow-down" title="下移" type="down"></i>',
        			'</span>'
    			].join('');
            }
        };
    ...
    DTree.prototype.menubarMethod = function(){
        ...
        return {
            ...
            openOrderModel: function() { // 打开排序模式
            	_this.obj.addClass('dtree-show-order');
            },
            closeOrderModel: function() { // 关闭排序模式
            	_this.obj.removeClass('dtree-show-order');
            }
        };
    };
    ...
    DTree.prototype.bindBrowserEvent = function(){
        ...
        // 绑定排序事件
        _this.obj.on("click", "div[dtree-click='"+eventName.itemNodeClick+"'][dtree-disabled='false'] ."+LI_DIV_ORDER_CLASS+">i", function(event) {
            event.stopPropagation();
            ...
            function doOrder() {
                if (type == 'top') {
                	if ($ul.children().first().data("id") == $li.data("id")) {
                		layer.msg('已经置顶了');
                		return;
                	}
                	$li.remove();
                	...
                	if ($ul.children().last().data("id") == $li.data("id")) {
                		layer.msg('已经置底了');
                		return;
                	}
                	$li.remove();
            ...
            // 排序事件执行完毕后，用户自定义自己的事件
            layui.event.call(this, MOD_NAME, "order("+$(_this.obj)[0].id+")",  {
                param: _this.callbackData().node(node),
                childParams: _this.callbackData().childNode($div),
                parentParam: _this.callbackData().parentNode($div),
                dom: _this.callbackData().dom($div),
